...
- **Everything real is a PBS job.** GPU jobs cap at 24 h (2 h in dev), so
  training chains itself: `bash scripts/platforms/aspire2a/train_in_pb.sh <config.yaml> <dataset>`
  (`--pilot` for a 2 h dev run). Submit from a **login node**, to `-q ai` --
  every AI queue is route-only, `qsub -q aiq1` is rejected.
- **Don't move or rename** `scripts/platforms/aspire2a/train_in_pb.sh` or the `*.pbs` scripts while
