...
| `DistStoreError: use_libuv was requested but PyTorch was built without libuv support` | A process group was initialised by hand with a `tcp://` init method; the image's torch has no libuv. Launch through `torchrun --standalone` (see [usage.md](usage.md#multi-gpu-training)), which works, or use a `file://` init method, as `test_distributed_training.py` does. |
