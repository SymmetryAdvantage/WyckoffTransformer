...
`scripts/train_in_pb.sh` resolves the locations **once, at submission**, writes
...
queued. `train_ehull_5x.pbs` and `train_ehull_ssops.pbs` resolve and export at the
top. Exporting matters for containers: the environment tier wins, so the Python
