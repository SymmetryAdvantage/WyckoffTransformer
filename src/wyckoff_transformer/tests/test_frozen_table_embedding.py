...
from ..wyckoff_processor import save_frozen_table
...
class TestFrozenTableEmbedding(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # A temporary directory, never the committed package data
        self.engineers_dir = Path(self.tmp.name)
        save_frozen_table("test_field", self.table, engineers_dir=self.engineers_dir)
    ...
    def test_matches_the_pass_through_vector_path(self):
        dense = CascadeEmbedding(self._cascade({"pass_through_vector": self.N_FEATURES}),
                                 engineers_dir=self.engineers_dir)
    ...
    def test_survives_an_optimiser_step(self):
        ...
        embedding = CascadeEmbedding(cascade, engineers_dir=self.engineers_dir)
