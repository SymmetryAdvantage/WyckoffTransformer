...
# Engineered field definitions and the frozen lookup tables that go with them are package
# data, generated by preprocess_wychoffs and shipped by the hatchling build hook.
...
def load_frozen_table(name: str, engineers_dir: Optional[Path] = None) -> np.ndarray:
    ...
    if not source.exists():
        raise FileNotFoundError(
            f"No frozen lookup table for {name!r} at {source}. Tables are generated by "
            "wyckoff_transformer.preprocess_wychoffs; run it, or reinstall the package.")
