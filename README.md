...
```
Alternatively, generation supports local model files inside the `runs` directory via the model path:
```bash
python scripts/generate.py <output-file> --model-path runs/<run-id>
```
