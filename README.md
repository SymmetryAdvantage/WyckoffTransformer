...
6. `wandb` library is used extensively and must be installed. Logging can ber disabled via `WANDB_MODE=disabled`. Otherwise, log into Wandb and configure your entity. Internally, we used `symmetry-advantage`. It can be configured via poetry:
```bash
