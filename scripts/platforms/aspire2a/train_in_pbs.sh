...
#
#     bash scripts/platforms/aspire2a/train_in_pbs.sh yamls/models/lemat_bulk_fmax1/gene_min_energy_adamw_wsd.yaml lemat_bulk_fmax1
#
...
submit_mode() {
  ...
  Pass --run-id $(cat "$RUNS_DIR/.$LEGACY_KEY.runid") to continue that run from branch '$BRANCH', or --fresh to start a new one."
    ...
    if [ "$FRESH" -eq 1 ]; then
        ...
        # The pin itself is dropped just before qsub, not here: a --dry-run must leave
        # it alone, or the next real submission silently starts over.
    ...
    # spec nothing will source nor repoints .latest.sh at one.
    local SPEC_TEXT
    SPEC_TEXT=$(
    ...
    local DROP_PIN=0
    if [ "$FRESH" -eq 1 ] && [ -n "$RUNID_FILE" ] && [ -f "$RUNID_FILE" ]; then
        DROP_PIN=1
    fi
    ...
    if [ "$DROP_PIN" -eq 1 ]; then
        # Last thing before qsub: the first link adopts any pin it finds, so --fresh
        # needs it gone, but a submission that dies before this point keeps it.
