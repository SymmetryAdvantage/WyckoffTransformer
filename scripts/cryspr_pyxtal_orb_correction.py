...
def build_orb_calculator(
    ...
    """Construct a default ORB ASE calculator with optional overrides."""
...
def generate_mp2020_compatible_entry(
        structure: Structure,
        total_energy: float,
) -> ComputedStructureEntry:
    """Create a MP2020-compatible entry using the default correction scheme."""
    # Generate VASP-like inputs to infer U values and run type
    builder = MPRelaxSet(structure)
    with tempfile.TemporaryDirectory() as tmpdirname:
        builder.write_input(f"{tmpdirname}/", potcar_spec=True)
    ...
    # Use default MP2020 correction scheme (no external config file)
...
def compute_mp2020_corrected_energy(
        atoms: Atoms,
        total_energy: float,
):
    """Return corrected total and per-atom energies using MP2020 scheme."""
    structure = AseAtomsAdaptor.get_structure(atoms)
    entry = generate_mp2020_compatible_entry(structure, total_energy)
    return entry.energy, entry.energy_per_atom, entry.correction
...
def func_run(
    ...
    energy_per_atom_corrected = None
    ...
    if len(atoms_relaxed_dict) == 0:
        ...
        try:
            energy_corrected, energy_per_atom_corrected, _ = compute_mp2020_corrected_energy(
                atoms,
                energy,
            )
        except Exception as exc:
            with open(f"{rootdir}/cryspr.log", mode='a+') as f:
                f.write(
                    f"[{now()}] CrySPR Error: Failed MP2020 correction for {model}-{id_gene} ({exc})\n"
                )
...
def main():
    ...
    ]].to_csv(output_csv_file, mode="w", index=False)
